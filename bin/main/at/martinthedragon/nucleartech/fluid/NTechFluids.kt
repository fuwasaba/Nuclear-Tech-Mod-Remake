...
object NTechFluids {
    ...
    val coolant = registerFluid("coolant", FluidAttributes.builder(ntm("fluid/coolant_still"), ntm("fluid/coolant_flow")).density(100_000).temperature(207 + ABSOLUTE_ZERO), ::Source, ::Flowing, ::BucketItem, propertiesModifier = { tickRate(30) }, fluidBlockProperties = BlockBehaviour.Properties.of(Material.WATER).jumpFactor(0.01F).speedFactor(0.01F).noCollission().strength(1000F).noDrops())
    val heavywater = registerFluid("heavywater", FluidAttributes.builder(ntm("fluid/heavywater_still"), ntm("fluid/heavywater_flow")).density(100_000).temperature(288 + ABSOLUTE_ZERO), ::Source, ::Flowing, ::BucketItem, propertiesModifier = { tickRate(30) }, fluidBlockProperties = BlockBehaviour.Properties.of(Material.WATER).jumpFactor(0.01F).speedFactor(0.01F).noCollission().strength(1000F).noDrops())
