...
class ReactorMk1Renderer(val context: BlockEntityRendererProvider.Context): RotatedBlockEntityRenderer<ReactorMk1BlockEntity>(context) {
    override fun renderRotated(blockEntity: ReactorMk1BlockEntity, model: SimpleRenderable, partials: Float, matrix: PoseStack, buffers: MultiBufferSource, light: Int, overlay: Int) {
        matrix.pushPose()
    ...
    private fun getTextureForFluid(react: ReactorMk1BlockEntity, fluid: FluidStack) = fluid.fluid.attributes.getFlowingTexture(fluid)
    override fun getModel(blockEntity: ReactorMk1BlockEntity)= SpecialModels.REACTOR_MK1.get()
}
